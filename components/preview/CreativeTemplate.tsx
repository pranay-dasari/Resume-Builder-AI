...
const LeftSection: React.FC<{title: string; children: React.ReactNode}> = ({ title, children }) => (
    <div className="mb-4" style={{ breakInside: 'avoid' }}>
        <h2 className="text-sm font-bold uppercase tracking-wider mb-2 pb-1 border-b-2 border-white/50">{title}</h2>
        {children}
    </div>
);
...
const RightSection: React.FC<{title: string; children: React.ReactNode}> = ({ title, children }) => (
    <div className="mb-6" style={{ breakInside: 'avoid' }}>
        <h2 className="text-sm font-bold uppercase tracking-wider text-gray-600 mb-3 pb-1 border-b-2 border-gray-300">{title}</h2>
        {children}
    </div>
);
...
const renderSummaryList = (text: string) => {
    return text.split('\n').map((line, index) => {
        if (line.trim().startsWith('*')) {
            return (
                <li key={index} className="relative pl-5 mb-1 text-sm" style={{ breakInside: 'avoid' }}>
...
const CreativeTemplate: React.FC<TemplateProps> = ({ data, settings }) => {
    ...
    const sectionComponents: Record<ReorderableSectionKey, React.ReactNode> = {
        summary: summary ? (
            <RightSection title="Summary">
                <div className="text-sm">{renderSummaryList(summary)}</div>
            </RightSection>
        ) : null,
        ...
        education: education.length > 0 ? (
            <LeftSection title="Education">
                {education.map(edu => (
                    <div key={edu.id} className="mb-3 text-sm" style={{ breakInside: 'avoid' }}>
                        <h3 className="font-bold">{edu.degree}</h3>
                        <p className="">{edu.institution}</p>
                        <p className="text-xs opacity-80">{edu.areaOfStudy}</p>
                        ...
                        <div key={skill.id} className="mb-2">
                             ...
                             {skill.keywords.map((keyword, index) => (
                                 <li key={index} className="relative pl-4" style={{ breakInside: 'avoid' }}>
                                    <span className="absolute left-0 top-1.5 w-1.5 h-1.5 bg-white/80 rounded-full"></span>
                                    {keyword}
                                </li>
                             ))}
                        </div>
        ...
        profiles: profiles.length > 0 ? (
            <LeftSection title="Profiles">
                {profiles.map(p => (
                    <a key={p.id} href={formatUrl(p.url)} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm mb-1 hover:opacity-80 break-words">
        ...
        certifications: certifications.length > 0 ? (
            <LeftSection title="Certifications">
                {certifications.map(c => (
                     <div key={c.id} className="mb-2 text-sm" style={{ breakInside: 'avoid' }}>
                        <p className="font-semibold">{c.name}</p>
                        <p className="text-xs opacity-80">{c.issuer} ({c.date})</p>
                    </div>
                ))}
            </LeftSection>
        ) : null,
        languages: languages.length > 0 ? (
            <LeftSection title="Languages">
                 {languages.map(lang => (
                     <p key={lang.id} className="text-sm">{lang.language} <span className="opacity-80">({lang.fluency})</span></p>
                 ))}
            </LeftSection>
        ) : null,
    ...
    return (
        ...
        >
            ...
            {/* Left Column */}
            <div className="w-[35%] p-6" style={{ backgroundColor: colors.primary, color: 'white' }}>
                <div className="mb-6">
                    <h1 className="text-3xl font-bold uppercase tracking-wide">{basics.name}</h1>
                    <p className="text-lg opacity-90">{basics.headline}</p>
                </div>
            ...
            {/* Right Column */}
            <div className="w-[65%] p-8" style={{ color: colors.text }}>
